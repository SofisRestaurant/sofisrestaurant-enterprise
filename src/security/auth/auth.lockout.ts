...
import { AUTH_WARN_AT_REMAINING_ATTEMPTS, AUTH_LOCKOUT_THRESHOLDS } from './auth.constants';
...
export function formatLockoutMessage(tier: number, remainingSeconds: number): string {
  ...
  switch (tier) {
    case 1:
      return `Too many attempts. Please wait ${duration} before trying again.`;
    case 2:
      return `Account temporarily locked. Try again in ${duration}.`;
    case 3:
      return `Account locked due to repeated failures. Please wait ${duration}.`;
    case 4:
    default:
      return `Account locked for ${duration} due to too many failed attempts. If this is your account, use "Forgot Password" to regain access.`;
  }
}
...
export function interpretServerAuthError(errorMessage: string): {
  ...
  switch (code) {
    ...
    case 'over_email_send_rate_limit':
      return {
        message: 'Too many emails sent. Please check your inbox or wait a few minutes.',
        isLockout: true,
        isInvalid: false,
        isBanned: false,
      };
