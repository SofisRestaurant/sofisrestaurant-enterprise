...
// - READ:  menu_items table (menu_items_public view was dropped; use RPC for
//          modifier-aware public reads — see MenuPublicService)
...
// menu_items_public view is dropped — use the table row type directly.
...
function sanitizeString(value: string, field: string, max = 300): string {
  ...
  if (!normalized) throw new MenuServiceError(`${field} is required`);
  if (normalized.length > max) throw new MenuServiceError(`${field} is too long`);
  return normalized;
}
...
function sanitizeOptionalInt(
  ...
  if (!Number.isFinite(value)) throw new MenuServiceError(`${field} must be a valid number`);
...
function sanitizePrice(value: number, field = 'price'): number {
  if (!Number.isFinite(value)) throw new MenuServiceError(`${field} must be a valid number`);
  if (value < 0) throw new MenuServiceError(`${field} must be non-negative`);
...
function sanitizeNullableStringArray(
  ...
  if (value === undefined) return undefined;
  if (value === null) return null;
  if (!Array.isArray(value)) throw new MenuServiceError(`${field} must be an array`);
  ...
  const normalized = Array.from(
    new Set(
      value
        ...
        .filter(Boolean)
        .map((entry) => {
          if (entry.length > maxItemLen) throw new MenuServiceError(`${field} contains an item that is too long`);
          return entry;
        }),
    ),
  );

  if (normalized.length > maxItems) throw new MenuServiceError(`${field} has too many entries`);
  return normalized;
}
...
function sanitizeWritePayload(payload: MenuItemWritePayload): MenuItemInsert {
  return {
    ...
    sort_order: sanitizeOptionalInt(payload.sort_order, 'sort_order', { min: 0, max: MAX_SORT_ORDER }),
    spicy_level: sanitizeOptionalInt(payload.spicy_level, 'spicy_level', { min: 0, max: MAX_SPICY_LEVEL }),
    ...
    allergens: sanitizeNullableStringArray(payload.allergens, 'allergens', MAX_TAG_ITEMS, MAX_TAG_ITEM_LENGTH) ?? null,
    inventory_count: sanitizeOptionalInt(payload.inventory_count, 'inventory_count', { min: 0, max: MAX_STOCK }),
    low_stock_threshold: sanitizeOptionalInt(payload.low_stock_threshold, 'low_stock_threshold', { min: 0, max: MAX_STOCK }),
    popularity_score: sanitizeOptionalInt(payload.popularity_score, 'popularity_score', { min: 0, max: MAX_STOCK }),
    pairs_with: sanitizeNullableStringArray(payload.pairs_with, 'pairs_with', MAX_TAG_ITEMS, MAX_TAG_ITEM_LENGTH) ?? null,
  };
}
...
function sanitizePartialWritePayload(payload: Partial<MenuItemWritePayload>): MenuItemUpdate {
  ...
  if (payload.name !== undefined)        update.name = sanitizeString(payload.name, 'name', MAX_NAME_LENGTH);
  if (payload.category !== undefined)    update.category = payload.category;
  if (payload.price !== undefined)       update.price = sanitizePrice(payload.price);
  if (payload.description !== undefined) update.description = sanitizeNullableString(payload.description, 'description', MAX_LONG_TEXT_LENGTH);
  if (payload.image_url !== undefined)   update.image_url = sanitizeNullableString(payload.image_url, 'image_url', MAX_URL_LENGTH);
  if (payload.available !== undefined)   update.available = Boolean(payload.available);
  if (payload.featured !== undefined)    update.featured = Boolean(payload.featured);
  if (payload.sort_order !== undefined)  update.sort_order = sanitizeOptionalInt(payload.sort_order, 'sort_order', { min: 0, max: MAX_SORT_ORDER });
  if (payload.spicy_level !== undefined) update.spicy_level = sanitizeOptionalInt(payload.spicy_level, 'spicy_level', { min: 0, max: MAX_SPICY_LEVEL });
  if (payload.is_vegan !== undefined)        update.is_vegan = sanitizeOptionalBoolean(payload.is_vegan) ?? null;
  if (payload.is_vegetarian !== undefined)   update.is_vegetarian = sanitizeOptionalBoolean(payload.is_vegetarian) ?? null;
  if (payload.is_gluten_free !== undefined)  update.is_gluten_free = sanitizeOptionalBoolean(payload.is_gluten_free) ?? null;

  if (payload.allergens !== undefined) {
    update.allergens = sanitizeNullableStringArray(payload.allergens, 'allergens', MAX_TAG_ITEMS, MAX_TAG_ITEM_LENGTH) ?? null;
  }
  if (payload.inventory_count !== undefined) {
    update.inventory_count = sanitizeOptionalInt(payload.inventory_count, 'inventory_count', { min: 0, max: MAX_STOCK });
  }
  if (payload.low_stock_threshold !== undefined) {
    update.low_stock_threshold = sanitizeOptionalInt(payload.low_stock_threshold, 'low_stock_threshold', { min: 0, max: MAX_STOCK });
  }
  if (payload.popularity_score !== undefined) {
    update.popularity_score = sanitizeOptionalInt(payload.popularity_score, 'popularity_score', { min: 0, max: MAX_STOCK });
  }
  if (payload.pairs_with !== undefined) {
    update.pairs_with = sanitizeNullableStringArray(payload.pairs_with, 'pairs_with', MAX_TAG_ITEMS, MAX_TAG_ITEM_LENGTH) ?? null;
  }
...
function toGatewayPayload(payload: AdminGatewayMenuItemPayload): MenuItemInsert | MenuItemUpdate {
...
async function callAdminGateway<T>(action: AdminGatewayAction, payload: UnknownRecord): Promise<T> {
  try {
    return await invokeEdge<T>('admin-gateway', { action, payload });
  } catch (error) {
...
function extractMenuItemIdFromGatewayResult(result: unknown, fallbackId?: string): string | null {
  if (typeof fallbackId === 'string' && fallbackId.trim().length > 0) return fallbackId;
  if (hasStringId(result)) return result.id.trim();
  return null;
}
...
export class MenuService {
  /** All available items (public). Reads from menu_items table directly. */
  ...
  /** Public single-item lookup. Reads from menu_items table directly. */
  ...
  static async updateMenuItem(id: string, payload: Partial<MenuItemWritePayload>): Promise<MenuItemAdmin> {
    ...
    await callAdminGateway<unknown>('menu:items:update', { id: safeId, ...sanitized });
  ...
  static async deleteMenuItem(id: string): Promise<void> {
    const safeId = ensureItemId(id);
    await callAdminGateway<unknown>('menu:items:delete', { id: safeId });
  }
