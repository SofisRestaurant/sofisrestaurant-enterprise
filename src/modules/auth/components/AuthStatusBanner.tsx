...
export function AuthStatusBanner({
  ...
  const bannerCopy = useMemo(() => {
    if (state.snapshot?.status === 'authenticated') {
      ...
      return {
        ...
        subtitle:
          subtitle ??
          `Your session is active as ${email}. You can continue securely from here.`,
      };
    }
    ...
    return {
      ...
      subtitle:
        subtitle ??
        'Save your progress, manage orders, and access loyalty features from one account.',
    };
  ...
  if (state.loading) {
    return (
      <section
        className={cx(
          'rounded-2xl border border-zinc-800 bg-[#050509] p-4 text-zinc-100',
  ...
  if (state.snapshot?.status === 'authenticated' && !showWhenAuthenticated) {
    return null;
  }

  if (state.snapshot?.status !== 'authenticated' && !showWhenAnonymous) {
    return null;
  }
  ...
  return (
    <section
      className={cx(
        'rounded-2xl border border-zinc-800 bg-[#050509] text-zinc-100 shadow-[0_0_0_1px_rgba(15,23,42,0.9)]',
