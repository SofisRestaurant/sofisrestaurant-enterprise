...
function sanitizePlainText(value: unknown, maxLength: number): string | null {
  ...
  const withoutControls = trimmed.replace(/[\u0000-\u001F\u007F]/g, ' ').replace(/\s+/g, ' ').trim()
...
async function fetchActiveCampaigns(
  ...
  const inflightPromise = (async (): Promise<CacheEntry> => {
    ...
    try {
      const response = await supabase.functions.invoke(buildFunctionPath(placement, limit, featured), {
        method: 'GET',
      })

      if (response.error) {
        const fallback = previous ??
          createEmptyEntry(placement, ERROR_TTL_MS, ERROR_TTL_MS, getErrorMessage(response.error), 'FETCH_FAILED')
      ...
      if (!parsed) {
        const fallback = previous ??
          createEmptyEntry(placement, ERROR_TTL_MS, ERROR_TTL_MS, 'Invalid active campaigns response.', 'INVALID_RESPONSE')
      ...
      const fallback = previous ??
        createEmptyEntry(placement, ERROR_TTL_MS, ERROR_TTL_MS, getErrorMessage(error), 'FETCH_FAILED')
...
export async function prefetchActiveCampaigns(input?: string | UseActiveCampaignsOptions): Promise<CampaignPublic[]> {
  const normalized = normalizeOptions(input)
...
export function getCachedActiveCampaigns(input?: string | UseActiveCampaignsOptions): CampaignPublic[] {
  const normalized = normalizeOptions(input)
  return readCache(getCacheKey(normalized.placement, normalized.limit, normalized.featured))?.campaigns ?? []
}

export function useActiveCampaignsState(input?: string | UseActiveCampaignsOptions): UseActiveCampaignsState {
  const normalized = normalizeOptions(input)
  const { placement, limit, featured, enabled, ttlMs, staleMs, refreshOnWindowFocus, refreshOnReconnect, pollIntervalMs } = normalized
  ...
  const [state, setState] = useState<UseActiveCampaignsState>(() => {
    const cached = readCache(key)
    return toHookState(cached, placement, enabled && cached === null, false, cached !== null, refreshFn)
  })
  ...
  const refresh = useCallback(async (): Promise<void> => {
    ...
    setState(toHookState(cached, placement, !hasCachedData, hasCachedData, cached !== null, refreshFn))
