...
// - ✅ Add-to-cart is fail-closed unless preflight ok AND modifier constraints satisfied
// - ✅ Modifier groups fetched best-effort from menu_items_public (no business truth assumed)
// - ✅ Inventory gating for modifiers (blocks unavailable selections)
// - ✅ Premium restaurant UX: hero image, chips, grouped modifiers, sticky CTA bar
// - ✅ A11y: focus management hints, ARIA, keyboard-friendly controls
// =============================================================================
...
export default function MenuItemModal({ item, onClose }: Props) {
  ...
  const loadModifierGroups = useCallback(async () => {
    ...
    try {
      // Try the view name that exists in your repo and migrations.
      // If your project renamed it, this will fail gracefully.
      ...
      // Expand required groups by default (premium UX).
  ...
  const setSelectionForGroup = useCallback((group: ModifierGroupLike, mod: ModifierLike) => {
    ...
    setSelected((prev) => {
      ...
      if (group.type === 'radio') {
        if (exists) {
          // allow deselect only if optional
          const min = group.min_selections ?? (group.required ? 1 : 0);
      ...
      // checkbox: toggle with max enforcement
      const next = exists
      ...
      if (max != null && max > 0 && next.length > max) {
        // if over max, drop oldest selection (predictable)
  ...
  const handleAddToCart = useCallback(() => {
    ...
    addTimer.current = setTimeout(() => {
      // Flatten selections to CartModifier-compatible array
      ...
      if (successTimer.current) clearTimeout(successTimer.current);
      successTimer.current = setTimeout(() => onClose(), 900);
    }, 180);
  }, [
    canAdd,
    preflight,
    ...
    <div className="text-white">
      ...
      <div className="sticky top-0 z-10 bg-neutral-950/80 backdrop-blur supports-backdrop-filter:bg-neutral-950/60">
        <div className="flex items-center justify-between px-5 pt-5">
          ...
          <button
            type="button"
            onClick={onClose}
            className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-white hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-amber-500/25"
            aria-label="Close"
          >
        ...
        <div className="px-5 pb-5 pt-4">
          <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-white/5">
            ...
            <div className="absolute bottom-3 left-3">
              <div className="rounded-2xl border border-amber-500/20 bg-neutral-950/60 px-3 py-2 backdrop-blur">
                ...
                {extrasLabel ? (
                  <p className="mt-0.5 text-[11px] text-zinc-400">{extrasLabel}</p>
                ) : null}
              </div>
            </div>
          </div>
      ...
      <div className="px-5 pb-28">
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            {modifierGroups.length ? (
              <button
                ...
                className="shrink-0 rounded-lg px-2 py-1 text-xs font-semibold text-zinc-300 hover:bg-white/5 focus:outline-none focus:ring-2 focus:ring-amber-500/25"
              >
                Clear
              </button>
            ) : null}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <button
                    ...
                    className="mt-3 rounded-lg bg-white/10 px-3 py-2 text-xs font-semibold text-white hover:bg-white/15 focus:outline-none focus:ring-2 focus:ring-amber-500/25"
                  >
                    Retry
                  </button>
                </div>
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left hover:bg-white/3 focus:outline-none focus:ring-2 focus:ring-amber-500/25"
        ...
        <div className="mt-6">
          ...
          />
          <p className="mt-1 text-[11px] text-zinc-500">{clampInt(notes.length, 0, 999)} / 600</p>
        </div>
      ...
      {/* Sticky footer */}
      <div className="fixed inset-x-0 bottom-0 z-20 border-t border-white/10 bg-neutral-950/85 backdrop-blur supports-backdrop-filter]:bg-neutral-950/65">
        <div className="mx-auto w-full max-w-2xl px-5 py-4">
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <button
              type="button"
              className={cx(
                'h-12 rounded-2xl px-5 text-sm font-semibold transition focus:outline-none focus:ring-2 focus:ring-amber-500/25',
                canAdd
              ...
              aria-disabled={!canAdd || phase !== 'idle' ? 'true' : 'false'}
            >
              {preflightLoading
                ? 'Checking…'
                : phase === 'adding'
