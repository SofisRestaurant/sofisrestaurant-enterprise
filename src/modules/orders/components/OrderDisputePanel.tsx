...
type DisputeUrgency = OrderDisputeSummary['disputeUrgency'];

// The kind values accepted by OrderTimeline — mirrors its own union so we
// avoid `as any` when mapping dispute event types.
...
function toTimelineKind(eventType: string): TimelineEventKind {
  if (
...
const URGENCY_CFG: Record<string, UrgencyCfg> = {
  overdue: {
    bg: 'bg-rose-900/30',
...
} satisfies Record<string, UrgencyCfg>;

const URGENCY_LABEL: Record<string, string> = {
  overdue: 'Response overdue',
  critical: 'Response due very soon',
  warning: 'Response due soon',
  normal: 'Response pending',
};
...
function UrgencyBanner({
  ...
  const { bg, border, text, icon } = URGENCY_CFG[urgency] ?? URGENCY_CFG.normal;
  const label = URGENCY_LABEL[urgency] ?? 'Response pending';

  return (
    <div className={['flex items-center gap-3 px-4 py-3 rounded-xl border', bg, border].join(' ')}>
      <span className={text}>{icon}</span>
      <div>
        <p className={['text-sm font-semibold', text].join(' ')}>{label}</p>
...
const EVIDENCE_STATUS_CFG: Record<string, string> = {
...
function EvidenceStatusBadge({ status }: { status: string }) {
  const cls = EVIDENCE_STATUS_CFG[status] ?? EVIDENCE_STATUS_CFG.not_started;
  return (
    <span
      className={[
    ...
    <div className="flex flex-col items-center justify-center py-10 gap-3">
      ...
      <div className="text-center">
        ...
        <p className="text-xs text-slate-500 mt-1">
          Dispute events will appear here if Stripe notifies us.
        </p>
      </div>
    </div>
  );
...
// ---------------------------------------------------------------------------
// Urgency → days-left text color
// ---------------------------------------------------------------------------

function urgencyTextColor(urgency: DisputeUrgency): string {
  if (urgency === 'overdue' || urgency === 'critical') return 'text-rose-400';
  if (urgency === 'warning') return 'text-amber-400';
    ...
    <div className={['space-y-5', className].join(' ')}>
      ...
      <div className="rounded-xl border border-white/8 bg-white/3 px-4 py-2">
        ...
        <InfoRow label="Disputed amount">
          <span className="font-semibold text-rose-300">{dispute.disputeAmountFormatted}</span>
        </InfoRow>
            ...
            {dispute.disputeDueDaysLeft !== null && (
              <span
                className={[
                  ...
                  urgencyTextColor(dispute.disputeUrgency),
                ].join(' ')}
              >
        ...
        <div className="rounded-xl border border-white/8 bg-white/3 p-4">
          ...
          <OrderTimeline
            events={dispute.events.map((ev) => ({
              id: ev.id,
              kind: toTimelineKind(ev.eventType),
              severity:
                ev.eventType === 'dispute_created'
                  ? 'error'
                  : ev.eventType === 'dispute_closed' && ev.newStatus === 'won'
                    ? 'success'
                    : ev.eventType === 'evidence_submitted'
                      ? 'info'
                      : 'neutral',
