...
function sanitizeItems(items: unknown): unknown[] {
  ...
  return sliced.map((raw) => {
    ...
    const modsRaw = Array.isArray(r.modifiers) ? r.modifiers.slice(0, GUARDS.MAX_MODIFIERS_PER_ITEM) : []
    const modifiers = modsRaw.map((m) => {
      ...
      const adj = mr.priceAdjustment ?? mr.priceAdjustmentCents ?? (mr as any).price_adjustment ?? 0
      const priceAdjustment = clampCentsSigned(adj, -GUARDS.MAX_UNIT_PRICE_CENTS, GUARDS.MAX_UNIT_PRICE_CENTS, 0)
      return {
        ...mr,
        priceAdjustment,
...
function computeSuspicionFlags(t: {
  ...
  const inconsistent =
    t.totalCents < afterCredit || t.taxCents > t.totalCents || t.taxCents < 0
...
function MoneyRow({ label, valueCents, strong, muted, testId }: MoneyRowProps) {
  const labelCls = strong ? 'text-gray-900 font-semibold' : muted ? 'text-gray-500' : 'text-gray-600'
  const valueCls = strong ? 'text-gray-900 font-semibold' : muted ? 'text-gray-600' : 'text-gray-900'
...
export function CartSummary() {
  ...
  const { totals, flags } = useMemo(() => {
    ...
    const computed = computeCartTotals(safeItems as any, safePromo as any, safeCredit as any, safeRate)
  ...
  return (
    <div className="space-y-2 rounded-lg border border-gray-200 bg-white p-4">
      ...
      {hasDiscount ? (
        <MoneyRow label="Discount" valueCents={-totals.discountCents} muted testId="cart-discount" />
      ) : null}
      ...
      <div className="space-y-2 border-t border-gray-200 pt-2">
        ...
        <p className="text-[11px] leading-snug text-gray-500">
          Final total is confirmed at secure payment (Stripe) and may include promotions, credits, and tax.
        </p>

        {flags.inconsistent ? (
          <p className="text-[11px] leading-snug text-red-600">
            Pricing estimate looks inconsistent. Please refresh the page. Final total will be confirmed at payment.
          </p>
        ) : flags.suspicious ? (
