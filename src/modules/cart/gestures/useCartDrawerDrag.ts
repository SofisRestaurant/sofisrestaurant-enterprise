...
export function useCartDrawerDrag({
  ...
  const releasePointerCaptureSafe = useCallback((target: EventTarget, id: number) => {
    try {
      (target as HTMLElement).releasePointerCapture(id);
    } catch {
      // Pointer capture may already be released by the browser during cancel/up.
    }
  }, []);
  ...
  }, [getSheet, onClose, clearInlineStyles, releasePointerCaptureSafe]);
  ...
  }, [getSheet, clearInlineStyles, releasePointerCaptureSafe]);
