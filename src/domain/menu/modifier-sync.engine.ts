...
export class ModifierSyncEngine {
   ...
   */
  static processEvent(event: RealtimeModifierEvent_, currentGroups: ModifierGroup[]): SyncResult {
    ...
    if (event.table === 'modifier_groups') {
      return ModifierSyncEngine.processGroupEvent(event, result);
    } else if (event.table === 'modifiers') {
      return ModifierSyncEngine.processModifierEvent(event, currentGroups, result);
    }
