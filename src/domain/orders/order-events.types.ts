...
export type EventBuilder<T extends OrderEventType> = {
  ...
  event_data: Extract<
    OrderEventData,
    T extends 'ORDER_CREATED'
      ? OrderCreatedEventData
      : T extends 'STATUS_CHANGED'
        ? StatusChangeEventData
        : T extends 'COOK_ASSIGNED'
          ? AssignmentEventData
          : T extends 'PAYMENT_RECEIVED' | 'PAYMENT_FAILED'
            ? PaymentEventData
            : T extends 'ISSUE_REPORTED' | 'ISSUE_RESOLVED'
              ? IssueEventData
              : T extends 'DELAY_REPORTED'
                ? DelayEventData
                : T extends 'ITEMS_MODIFIED'
                  ? ItemsModifiedEventData
                  : T extends 'NOTE_ADDED'
                    ? NoteEventData
                    : T extends 'PREPARING_STARTED' | 'PREPARING_COMPLETED'
                      ? KitchenWorkflowEventData
                      : T extends 'CUSTOMER_NOTIFIED'
                        ? CustomerNotificationEventData
                        : Record<string, unknown>
  >;
};
...
export function getEventTypesByCategory(
  category: 'lifecycle' | 'kitchen' | 'fulfillment' | 'issues' | 'modifications',
): OrderEventType[] {
  const categories = {
    lifecycle: [
