...
export function mergeGatewaySnapshots(
  ...
  if (layoutRes) {
    ...
    else out = applyLayoutSnapshot(out, (layoutRes.data ?? null) as Partial<AdminLayoutSnapshot> | null)
  }

  if (metricsRes) {
    if (isGatewayErr(metricsRes)) debugGatewayFailure(metricsRes, 'admin-gateway:metrics')
    else out = applyExecutiveSnapshot(out, (metricsRes.data ?? null) as Partial<ExecutiveSnapshot> | null)
  }
