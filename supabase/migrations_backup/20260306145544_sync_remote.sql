...
-- growth_campaigns_rotation_idx intentionally not created here.
-- Canonical creation is in 20960306170000_restore_active_campaigns_and_rotation.sql
-- after eligible_for_rotation is added to the table in that same file.
...
-- =============================================================================
-- REPLAY-SAFETY PATCH: v2_award_points 5-param overload
-- =============================================================================
...
BEGIN
  ...
  INSERT INTO loyalty_ledger (
    ...
    admin_id, idempotency_key, reference_id,
    tier_at_time, streak_at_time, metadata
  ) VALUES (
    p_account_id, v_points, v_new_balance, 'earn', 'order',
    p_admin_id, p_idempotency_key, p_reference_id,
    v_old_tier, v_old_streak,
    jsonb_build_object('amount_cents', p_amount_cents, 'order_id', p_reference_id)
  )
  ...
  IF v_rows_inserted = 0 THEN
    ...
    RETURN QUERY SELECT v_new_balance, v_new_lifetime, v_new_tier, false, true;
    RETURN;
  END IF;

  UPDATE loyalty_accounts
  SET tier = v_new_tier, last_activity = CURRENT_DATE
  WHERE id = p_account_id;
...
COMMENT ON FUNCTION public.v2_award_points(uuid, uuid, integer, text, uuid) IS
...
-- =============================================================================
-- REPLAY-SAFETY PATCH: purge non-canonical 4-param overload
-- =============================================================================
DROP FUNCTION IF EXISTS public.v2_award_points(uuid, uuid, integer, text);
