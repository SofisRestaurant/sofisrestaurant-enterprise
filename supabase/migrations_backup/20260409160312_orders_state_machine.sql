...
ALTER TABLE public.orders
  ...
  ADD COLUMN IF NOT EXISTS pending_cart_id UUID REFERENCES public.pending_carts(id) ON DELETE SET NULL;
...
CREATE INDEX IF NOT EXISTS idx_orders_pending_cart_id
  ON public.orders (pending_cart_id)
  WHERE pending_cart_id IS NOT NULL;
