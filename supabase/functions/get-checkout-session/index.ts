...
async function checkRateLimit(
  ...
  if (upErr)
    log('warn', 'rate_limit_upsert_failed', {
      requestId,
      userId: prefix(userId),
      msg: upErr.message,
    });
...
async function authenticateAndAuthorize(args: {
  ...
  | {
      ok: false;
      reason: 'missing_auth' | 'auth_failed' | 'rate_limited' | 'owner_mismatch' | 'stripe_error';
    }
...
> {
  ...
  try {
    ...
    log('warn', 'stripe_retrieve_failed', {
      requestId,
