...
function corsHeaders(origin: string | null): Record<string, string> {
  const allowed = origin && (ALLOWED_ORIGINS as readonly string[]).includes(origin) ? origin : "null";
  return {
...
Deno.serve(async (req) => {
  ...
  const { data: promo, error: promoError } = await svc
    .from("promotions")
    .select("id,type,value,active,starts_at,ends_at,expires_at,max_uses,per_user_limit,min_order_cents")
  ...
  if (discountType === "percent") {
    ...
    const fixedCents = clampInt(Math.round(discountValue * 100), 0, CONFIG.MAX_CART_TOTAL_CENTS);
