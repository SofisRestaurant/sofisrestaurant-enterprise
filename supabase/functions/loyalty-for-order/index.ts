...
function withRateHint(headers: Record<string, string>, code: string): Record<string, string> {
  ...
  if (code === "AUTH_MISSING" || code === "AUTH_INVALID") return { ...headers, "X-Auth-Result": code };
  return headers;
}
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  if (req.method !== "POST") {
    ...
    return json(chBase, { ok: false, error: pub.message, code: "METHOD_NOT_ALLOWED", meta }, pub.status);
  }
  ...
  if (!orderId) {
    ...
    return json(chBase, { ok: false, error: pub.message, code: "MISSING_ORDER_ID", meta }, pub.status);
  }
  if (!isUuid(orderId)) {
    const pub = safePublicError("INVALID_ORDER_ID");
    return json(chBase, { ok: false, error: pub.message, code: "INVALID_ORDER_ID", meta }, pub.status);
  }
  ...
  if (!ownerId) {
    log("warn", "order_not_found_or_unreadable", { requestId, userId: prefixId(userId), order: prefixId(orderId) });
    const pub = safePublicError("ORDER_NOT_FOUND");
    return json(chBase, { ok: false, error: pub.message, code: "ORDER_NOT_FOUND", meta }, pub.status);
  }

  if (ownerId !== userId) {
    log("warn", "forbidden_order", { requestId, userId: prefixId(userId), order: prefixId(orderId) });
  ...
  try {
    ...
    const accountId =
      acctRes.data && isRecord(acctRes.data) && typeof acctRes.data.id === "string" ? acctRes.data.id : null;
