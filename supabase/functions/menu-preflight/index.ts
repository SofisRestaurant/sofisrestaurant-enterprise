...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const stockCount =
      data.inventory_count == null
        ? null
        : clampInt(data.inventory_count, 0, CONFIG.MAX_STOCK);
    ...
    const inventoryMax = stockCount == null ? CONFIG.DEFAULT_MAX_QTY : Math.max(1, Math.min(CONFIG.MAX_QTY, stockCount));
