...
function structuredLog(outcome: LogOutcome, orderId: string, event: string, detail: Record<string, unknown>): void {
...
Deno.serve(async (req: Request) => {
  ...
  if (!supabaseUrl || !serviceKey) return jsonResponse(req, { ok: false, error: "Service misconfigured" }, 503);
